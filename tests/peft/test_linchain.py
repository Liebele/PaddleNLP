...
class TestLinChainLayer(unittest.TestCase):
    ...
    def test_forward(self):
        lora_layer = LoRALinear(
            in_features=16, out_features=8, r=4, lora_dropout=0.1, lora_alpha=8, lora_use_mixer=True, mixer_num=3
        )
    ...
    def test_train_eval(self):
        x = paddle.randn([2, 4, 16], "float32")
        lora_layer = LoRALinear(in_features=16, out_features=8, r=4, lora_use_mixer=True, mixer_num=3)
        lora_layer.train()
    ...
    def test_save_load(self):
        with TemporaryDirectory() as tempdir:
            ...
            new_lora_layer = LoRALinear(in_features=16, out_features=8, r=4, lora_use_mixer=True, mixer_num=3)
    ...
    def test_load_regular_linear(self):
        with TemporaryDirectory() as tempdir:
            ...
            lora_layer_r8 = LoRALinear(in_features=16, out_features=8, r=8, lora_use_mixer=True, mixer_num=3)
            lora_layer_r4 = LoRALinear(in_features=16, out_features=8, r=4, lora_use_mixer=True, mixer_num=3)
...
class TestLinChainModel(unittest.TestCase):
...
class TestLinChainConfig(unittest.TestCase):
    def test_save_load(self):
        with TemporaryDirectory() as tempdir:
            lora_config = LoRAConfig()
